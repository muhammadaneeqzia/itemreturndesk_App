 ...
 */
function mapPostImages(postImages) {
  return (postImages || [])
    .slice()
...
function formatPostRecord(data) {
  if (!data) return null;
  const profile = data.profiles;
  const images = mapPostImages(data.post_images);
  const videos = (data.post_videos || []).map((v) => v.video_url).filter(Boolean);
  ...
  return {
    ...
    tip: tipAmount != null && tipAmount !== '' ? `$${parseFloat(tipAmount).toFixed(2)}` : null,
...
export const postService = {
  ...
  async getPostsByType(type, filters = {}, limit = 20, offset = 0) {
    try {
      ...
      const formattedPosts = (data || []).map((post) => formatPostRecord(post));
