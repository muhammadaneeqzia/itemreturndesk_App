...
export const chatService = {
  ...
  /**
   * Get conversations tied to a specific post (owner sees all inbound chats).
   */
  async getConversationsForPost(postId, userId) {
    try {
      const { data, error } = await supabase
        .from('conversations')
        .select(`
          ...
          last_message_at,
          created_at,
          posts:post_id ( id, title, type ),
          user1:profiles!conversations_user1_id_fkey ( id, name, avatar_url, email ),
          user2:profiles!conversations_user2_id_fkey ( id, name, avatar_url, email )
        `)
        .eq('post_id', postId)
        .or(`user1_id.eq.${userId},user2_id.eq.${userId}`)
        .order('last_message_at', { ascending: false, nullsFirst: false });
      ...
      console.error('Get conversations for post error:', error);
      return { success: false, error: error.message || 'Failed to get conversations' };
    }
  },
