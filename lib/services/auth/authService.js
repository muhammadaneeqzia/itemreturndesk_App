...
import {
  ...
  isLikelyAuthRateLimitError,
  normalizeEmail,
} from '../../../utils';

function mapProfileToAppUser(profile) {
...
export const authService = {
  ...
  async signIn(email, password) {
    try {
      ...
      if (data.user) {
        ...
        return {
          success: true,
          user: mapProfileToAppUser(profile),
  ...
  /**
   * Persist profile fields (profiles row + auth email when it changes).
   * @param {string} userId
   * @param {{ name: string, phone?: string, email: string, bio?: string, location?: string }} fields
   * @returns {Promise<{ success: boolean, user?: object, error?: string }>}
   */
  async updateUserProfile(userId, fields) {
    try {
      const { data: { user: authUser }, error: authGetError } = await supabase.auth.getUser();
      if (authGetError || !authUser || authUser.id !== userId) {
        return { success: false, error: authGetError?.message || 'Not authenticated' };
      }
      ...
      if (normalizedEmail !== authUser.email) {
        ...
        if (emailError) {
          return { success: false, error: mapSupabaseAuthError(emailError) };
        }
      }
      ...
      if (fetchErr) {
        return { success: false, error: fetchErr.message || 'Failed to load profile' };
      }
      ...
      if (Object.prototype.hasOwnProperty.call(existing, 'bio')) {
      ...
      const updateResult = await userService.updateProfile(userId, payload);
